...
bool equal(){
	...
	for(it_q = q.begin(), it_stl = stl.begin(); it_q != q.end() || it_stl != stl.end(); it_q++, it_stl++){
		if(*it_q != *it_stl) {
			printf("cnt = %d\n",cnt);
			std::cout << it_q - q.begin() << std::endl;
			cnt = (*it_q != *it_stl);
			return 0;
		}
		cnt++;
...
void test3(){
	...
	for(int i =1 ; i <= 1000; i++)
	{
		int t1 = rand() % num;
		int t2 = rand() % num;
		if (*(q.begin() + t1) != *(stl.begin() + t1)) { puts("Wrong Answer1"); return; }
		if (t2 && *(q.end() - t2) != *(stl.end() - t2)) { puts("Wrong Answer2");  return; }
		if((q.begin() + t1) - (q.begin() + t2) != (t1 - t2)){
			puts("Wrong Answer3");
			std::cout << t1 << ' ' << t2 << ' ' << (q.begin() + t1) - (q.begin() + t2) << std::endl;
...
void test4(){
	...
	for(int i =1 ; i <= 1000; i++)
	{
		int t1 = rand() % num;
		int t2 = rand() % num;
		if(*(_q.cbegin() + t1) != *(_stl.cbegin() + t1)){puts("Wrong Answer1");return;}
		if(t2 && *(_q.cend() - t2) != *(_stl.cend() - t2)){puts("Wrong Answer2");return;}
		if((_q.cbegin() + t1) - (_q.cbegin() + t2) != (t1 - t2)){puts("Wrong Answer3");return;}
	}
